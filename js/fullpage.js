...
if(e.type==='touchend'){g_canFireMouseEnterNormalScroll=!1;setTimeout(function(){g_canFireMouseEnterNormalScroll=!0},800)}
...
var lastScrollTime=0;var scrollTimeDelay=100;function scrolling(type){if(!isScrollAllowed.m[type]){return}
var scrollSection=(type==='down')?moveSectionDown:moveSectionUp;if(options.scrollOverflow){var scrollable=options.scrollOverflowHandler.scrollable($(SECTION_ACTIVE_SEL)[0]);var check=(type==='down')?'bottom':'top';var currentScrollTime=new Date().getTime();var lastScrollTimeDiff=currentScrollTime-lastScrollTime;lastScrollTime=currentScrollTime;if(scrollable!=null){if(options.scrollOverflowHandler.isScrolled(check,scrollable)){if(lastScrollTimeDiff>scrollTimeDelay||window.innerWidth<640){scrollSection()}}else{return!0}}else{if(lastScrollTimeDiff>scrollTimeDelay||window.innerWidth<640){scrollSection()}}}else{scrollSection()}}
